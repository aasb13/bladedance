...
public:
	...
	ModResult OnStats(Stats::Context& stats) override
	{
		...
		for (auto* helper : helpop.helpers)
		{
			...
			if (helper->IsAway())
			{
				const std::string awayperiod = ToLongString(ServerInstance->Time() - helper->away->time, true);
			...
			if (lhelper)
			{
				const std::string idleperiod = ToLongString(ServerInstance->Time() - lhelper->idle_lastmsg, true);
