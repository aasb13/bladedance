	...
	: public ExtensionItem
{
	...
	void Delete(Extensible* container, void* item) override
	{
		...
		for (auto* user : dat->accepting)
		{
			...
			if (!target)
			{
				::Logs.Debug(MODNAME, "BUG: Inconsistency detected in callerid state, please report (1)");
				continue; // shouldn't happen, but oh well.
			}

			if (!stdalgo::vector::swaperase(target->wholistsme, dat))
				::Logs.Debug(MODNAME, "BUG: Inconsistency detected in callerid state, please report (2)");
		}
		delete dat;
	}
};
...
public:
	...
	bool RemoveAccept(User* user, User* whotoremove)
	{
		...
		if (!dat2)
		{
			...
			::Logs.Debug(MODNAME, "BUG: Inconsistency detected in callerid state, please report (3)");
			return false;
		}

		if (!stdalgo::vector::swaperase(dat2->wholistsme, dat))
			::Logs.Debug(MODNAME, "BUG: Inconsistency detected in callerid state, please report (4)");
	...
	void RemoveFromAllAccepts(User* who)
	{
		...
		for (auto* dat : userdata->wholistsme)
		{
			...
			if (!dat->accepting.erase(who))
				::Logs.Debug(MODNAME, "BUG: Inconsistency detected in callerid state, please report (5)");
		}
