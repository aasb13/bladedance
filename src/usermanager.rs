...
unsafe fn check_ping_timeout(um: *mut UserManager, user: *mut LocalUser) {
    ...
    if unsafe { um_ffi_server_time() } < unsafe { um_ffi_local_user_nextping(user) } {
        return;
    }

    // This user didn't answer the last ping, remove them.
    if unsafe { um_ffi_local_user_lastping(user) } == 0 {
        let t = unsafe { um_ffi_server_time() };
        let next = unsafe { um_ffi_local_user_nextping(user) };
        let pingtime = unsafe { um_ffi_local_user_class_pingtime(user) } as time_t;
        let secs_i64 = t - (next - pingtime);
        let secs_u64 = secs_i64.max(0) as u64;
        let msg = ping_timeout_quit_message(secs_u64);
        unsafe { quit_user_inner(um, user.cast::<User>(), msg.as_str(), None) };
        return;
    }

    unsafe { um_ffi_local_user_set_lastping(user, 0) };
    unsafe {
    ...
    let mut hook = unsafe { um_ffi_streamsocket_get_iohook(um_ffi_local_user_iohandler(user)) };
    while !hook.is_null() {
        if unsafe { um_ffi_iohook_ping(hook) } {
            return; // Client has been pinged.
        }
        hook = unsafe { um_ffi_iohook_next_in_chain(hook) };
    }

    // Send a ping to the client using an IRC message.
    unsafe { um_ffi_local_user_send_irc_ping(user) };
}

unsafe fn check_connection_timeout(um: *mut UserManager, user: *mut LocalUser) {
    let dl = unsafe { um_ffi_local_user_connection_timeout_deadline(user) };
    if dl >= 0 && unsafe { um_ffi_server_time() } > dl {
        // Either the user did not send NICK/USER or a module blocked connection in
        // OnCheckReady until the client timed out.
        unsafe { quit_user_inner(um, user.cast::<User>(), CONNECTION_TIMEOUT_MSG, None) };
    }
}

unsafe fn check_modules_ready(um: *mut UserManager, user: *mut LocalUser) {
    if unsafe { um_ffi_mod_on_check_ready_is_passthru(user) } {
        // User has sent NICK/USER and modules are ready.
        unsafe { um_ffi_local_user_full_connect(user) };
        return;
    }

    // If the user has been quit in OnCheckReady then we shouldn't quit
    // them again for having a registration timeout.
    if !unsafe { um_ffi_user_quitting(user.cast::<User>()) } {
        unsafe { check_connection_timeout(um, user) };
    }
}
...
unsafe fn quit_user_inner(
    ...
    if unsafe { um_ffi_user_quitting(user) } {
        let nick = unsafe { user_nick_string(user) };
        let nick_c = CString::new(nick).unwrap_or_else(|_| CString::new("").unwrap());
        unsafe { um_ffi_log_users_bug_quitting(nick_c.as_ptr()) };
        return;
    }

    if unsafe { um_ffi_user_is_server(user) } {
        let nick = unsafe { user_nick_string(user) };
        let nick_c = CString::new(nick).unwrap_or_else(|_| CString::new("").unwrap());
        unsafe { um_ffi_log_users_bug_server(nick_c.as_ptr()) };
        return;
    }

    let localuser = unsafe { um_ffi_user_as_local(user) };
    ...
    let quit_final = unsafe { c_str_to_string(um_ffi_quit_user_tls_quit()) };
    let oper_final = unsafe { c_str_to_string(um_ffi_quit_user_tls_oper()) };
    ...
    unsafe { um_ffi_user_set_quitting(user) };
    let uuid = unsafe { user_uuid_string(user) };
    let nick = unsafe { user_nick_string(user) };
    ...
    unsafe { um_ffi_log_quit_user(uuid_c.as_ptr(), nick_c.as_ptr(), log_quit_c.as_ptr()) };

    if !localuser.is_null() {
        unsafe { um_ffi_local_user_send_error_quit(localuser, oper_final_c.as_ptr()) };
    }

    unsafe { um_ffi_global_culls_add_item(user) };

    if unsafe { um_ffi_user_is_fully_connected(user) } {
        unsafe { um_ffi_foreach_mod_on_user_quit(user, quit_final_c.as_ptr(), oper_final_c.as_ptr()) };
        unsafe { um_ffi_write_common_quit(user, quit_final_c.as_ptr(), oper_final_c.as_ptr()) };
    } else {
        unsafe { um_ffi_user_manager_rust_access_dec_unknown(um) };
    }

    if !localuser.is_null() {
        unsafe { um_ffi_foreach_mod_on_user_disconnect(localuser) };
        unsafe { um_ffi_local_user_eh_close(localuser) };

        if unsafe { um_ffi_user_is_fully_connected(user) } {
            let mask = unsafe { user_real_mask_string(user) };
            let addr = unsafe { user_address_string_user(user) };
            ...
            unsafe { um_ffi_sno_write_client_exiting(mask_c.as_ptr(), addr_c.as_ptr(), oper_final_c.as_ptr()) };
        }
        unsafe { um_ffi_user_manager_rust_access_local_erase(um, localuser) };
        unsafe { um_ffi_local_user_connect_class_dec_use_count(localuser) };
    }

    let nick_erase = CString::new(nick.as_str()).unwrap_or_else(|_| CString::new("").unwrap());
    if !unsafe { um_ffi_user_manager_rust_access_client_erase_nick_cstr(um, nick_erase.as_ptr()) } {
        unsafe { um_ffi_log_users_bug_nick_not_found(nick_erase.as_ptr()) };
    }

    let uuid_erase = CString::new(uuid.as_str()).unwrap_or_else(|_| CString::new("").unwrap());
    unsafe { um_ffi_user_manager_rust_access_uuid_erase_cstr(um, uuid_erase.as_ptr()) };
...
pub unsafe extern "C" fn rust_usermanager_quit_user(
    ...
    let oper = if operquitmessage.is_null() {
        ...
        Some(unsafe { c_str_to_string(operquitmessage) })
    };
    unsafe { quit_user_inner(um, user, quit.as_str(), oper.as_deref()) };
}
...
pub unsafe extern "C" fn rust_usermanager_add_user(
    ...
    let new = unsafe { um_ffi_local_user_new(socket, client, server) };
    let eh = unsafe { um_ffi_local_user_iohandler(new) };

    unsafe { um_ffi_log_users_new_fd(socket) };

    unsafe { um_ffi_user_manager_rust_access_inc_unknown(um) };
    unsafe { um_ffi_user_manager_rust_access_client_insert(um, new) };
    unsafe { um_ffi_user_manager_rust_access_clonemap_add(um, new.cast::<User>()) };
    unsafe { um_ffi_user_manager_rust_access_local_push_front(um, new) };
    unsafe { um_ffi_foreach_mod_on_user_init(new) };

    if !unsafe { um_ffi_socket_engine_add_fd(eh, FD_WANT_FAST_READ | FD_WANT_EDGE_WRITE) } {
        unsafe { um_ffi_log_users_internal_error() };
        unsafe { quit_user_inner(um, new.cast::<User>(), ERR_INTERNAL_CONN, None) };
        return;
    }

    // If this listener has an IO hook provider set then tell it about the connection
    let n = unsafe { um_ffi_listen_socket_iohookprov_count(via) };
    for idx in 0..n {
        if !unsafe { um_ffi_listen_socket_iohookprov_valid(via, idx) } {
            if unsafe { um_ffi_listen_socket_iohookprov_empty_name(via, idx) } {
                continue;
            }

            unsafe { um_ffi_log_listen_iohook_nonexistent(via, idx) };
            let msg = unsafe { c_str_to_string(um_ffi_insp_format_misconfigured_iohook(idx)) };
            unsafe { quit_user_inner(um, new.cast::<User>(), msg.as_str(), None) };
            return;
        }

        unsafe { um_ffi_listen_socket_iohookprov_on_accept(via, idx, eh, client, server) };
        ...
        if unsafe { um_ffi_useriohandler_error_nonempty(eh) } {
            let err = unsafe { c_str_to_string(um_ffi_useriohandler_error_cstr(eh)) };
            unsafe { quit_user_inner(um, new.cast::<User>(), err.as_str(), None) };
            return;
        }
    }

    if unsafe { um_ffi_user_manager_rust_access_local_users_size(um) } > unsafe { um_ffi_config_soft_limit() } {
        unsafe { um_ffi_log_softlimit_warning() };
        unsafe { quit_user_inner(um, new.cast::<User>(), ERR_SOFT_LIMIT, None) };
        return;
    }

    if !unsafe { um_ffi_local_user_find_connect_class(new) } {
     ...
     */
    let exempt = !unsafe { um_ffi_xline_matches_E(new) }.is_null();
    unsafe { um_ffi_local_user_set_exempt_from_eline(new, exempt) };

    let addr = local_user_address_string(new);
    let addr_c = CString::new(addr.as_str()).unwrap_or_else(|_| CString::new("").unwrap());
    let b = unsafe { um_ffi_ban_cache_get_hit(addr_c.as_ptr()) };
    if !b.is_null() {
        if unsafe { um_ffi_bancache_hit_type_non_empty(b) } && !exempt {
            /* user banned */
            unsafe { um_ffi_log_bancache_positive(addr_c.as_ptr()) };

            if !unsafe { um_ffi_config_xline_message_empty() } {
                unsafe { um_ffi_local_user_write_numeric_banned(new) };
            }
            ...
            let reason = c_str_to_string(unsafe { um_ffi_bancache_hit_reason_cstr(b) });
            quit_user_inner(um, new.cast::<User>(), reason.as_str(), None);
            return;
        } else {
            unsafe { um_ffi_log_bancache_negative(addr_c.as_ptr()) };
        }
    } else if !exempt {
        let r = unsafe { um_ffi_xline_matches_Z(new) };
        if !r.is_null() {
            unsafe { um_ffi_xline_apply(r, new) };
            return;
        }
    }

    if unsafe { um_ffi_config_raw_log() } {
        unsafe { um_ffi_log_notify_raw_io(new) };
    }

    unsafe { um_ffi_foreach_mod_on_change_remote_address(new) };
    if !unsafe { um_ffi_user_quitting(new.cast::<User>()) } {
        unsafe { um_ffi_foreach_mod_on_user_post_init(new) };
    }
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_usermanager_add_clone(um: *mut UserManager, user: *mut User) {
    unsafe { um_ffi_user_manager_rust_access_clonemap_add(um, user) };
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_usermanager_remove_clone_counts(um: *mut UserManager, user: *mut User) {
    unsafe { um_ffi_user_manager_rust_access_clonemap_remove(um, user) };
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_usermanager_rehash_clone_counts(um: *mut UserManager) {
    unsafe { um_ffi_user_manager_rust_access_clonemap_clear(um) };
    let it = unsafe { um_ffi_user_manager_rust_access_client_iter_new(um) };
    loop {
        let u = unsafe { um_ffi_user_manager_rust_access_client_iter_next(it) };
        if u.is_null() {
            break;
        }
        unsafe { um_ffi_user_manager_rust_access_clonemap_add(um, u) };
    }
    unsafe { um_ffi_user_manager_rust_access_client_iter_free(it) };
}
...
pub unsafe extern "C" fn rust_usermanager_rehash_services(um: *mut UserManager) {
    ...
    let it = unsafe { um_ffi_user_manager_rust_access_client_iter_new(um) };
    loop {
        let user = unsafe { um_ffi_user_manager_rust_access_client_iter_next(it) };
        if user.is_null() {
            break;
        }
        if unsafe { um_ffi_user_server_is_service(user) } {
            vec.push(user);
        }
    }
    unsafe { um_ffi_user_manager_rust_access_client_iter_free(it) };
    if vec.is_empty() {
        unsafe { um_ffi_user_manager_rust_access_services_swap(um, ptr::null(), 0) };
    } else {
        unsafe { um_ffi_user_manager_rust_access_services_swap(um, vec.as_ptr(), vec.len()) };
    }
}
...
pub unsafe extern "C" fn rust_usermanager_do_background_user_stuff(um: *mut UserManager) {
    let it = unsafe { um_ffi_user_manager_rust_access_local_iter_new(um) };
    loop {
        let curr = unsafe { um_ffi_user_manager_rust_access_local_iter_next(it) };
        if curr.is_null() {
            break;
        }
        ...
        match unsafe { um_ffi_local_user_connected(curr) } {
            CONN_FULL => check_ping_timeout(um, curr),
            CONN_NICKUSER => check_modules_ready(um, curr),
            _ => check_connection_timeout(um, curr),
        }
    }
    unsafe { um_ffi_user_manager_rust_access_local_iter_free(it) };
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn rust_usermanager_next_already_sent_id(um: *mut UserManager) -> u64 {
    let mut id = unsafe { um_ffi_user_manager_rust_access_get_already_sent_id(um) };
    id = id.wrapping_add(1);
    unsafe { um_ffi_user_manager_rust_access_set_already_sent_id(um, id) };
    if id == 0 {
