...
BufferedSocketError BufferedSocket::BeginConnect(const irc::sockets::sockaddrs& dest, const irc::sockets::sockaddrs& bind, unsigned long timeout, sa_family_t protocol)
{
	...
	::Logs.Debug("SOCKET", "BufferedSocket::DoConnect success");
	return I_ERR_NONE;
}
...
void StreamSocket::DoWrite()
{
	...
	{
		::Logs.Debug("SOCKET", "DoWrite on errored or closed socket");
		return;
	}
...
void StreamSocket::WriteData(const std::string& data)
{
	if (!HasFd())
	{
		::Logs.Debug("SOCKET", "Attempt to write data to dead socket: {}",
			data);
		return;
	}
...
{
	::Logs.Debug("SOCKET", "SocketTimeout::Tick");
...
void StreamSocket::OnEventHandlerRead()
{
	...
	{
		::Logs.Normal("SOCKET", "Caught exception in socket processing on FD {} - '{}'",
			GetFd(), ex.GetReason());
...
void StreamSocket::CheckError(BufferedSocketError errcode)
{
	if (!error.empty())
	{
		::Logs.Debug("SOCKET", "Error on FD {} - '{}'", GetFd(), error);
		OnError(errcode);
	}
}
