...
ListenSocket::~ListenSocket()
{
	if (this->HasFd())
	{
		::Logs.Debug("SOCKET", "Shut down listener on fd {}", GetFd());
		SocketEngine::Shutdown(this, 2);

		if (SocketEngine::Close(this) != 0)
			::Logs.Warning("SOCKET", "Failed to close listener: {}", strerror(errno));

		if (bind_sa.family() == AF_UNIX && unlink(bind_sa.un.sun_path))
			::Logs.Warning("SOCKET", "Failed to unlink UNIX socket: {}", strerror(errno));
	}
}
...
void ListenSocket::OnEventHandlerRead()
{
	...
	if (incomingfd < 0)
	{
		::Logs.Debug("SOCKET", "Refused connection to {}: {}",
			bind_sa.str(), strerror(errno));
		ServerInstance->Stats.Refused++;
		return;
	}

	::Logs.Debug("SOCKET", "Accepted connection to {} with fd {}",
	...
	{
		::Logs.Debug("SOCKET", "Unable to get peer name for fd {}: {}",
	...
	ServerInstance->Stats.Refused++;
	::Logs.Normal("SOCKET", "Refusing connection on {} - {}", bind_sa.str(),
