...
class DummySerializer final : public ClientProtocol::Serializer {
  bool Parse(LocalUser *user, const std::string &line,
             ClientProtocol::ParseOutput &parseoutput) override {
    return false;
  }

  ClientProtocol::SerializedMessage
  Serialize(const ClientProtocol::Message &msg,
            const ClientProtocol::TagSelection &tagwl) const override {
    return {};
  }

public:
  DummySerializer(Module *mod) : ClientProtocol::Serializer(mod, "dummy") {}
};

class CommandReloadmodule final : public Command {
...
public:
  CommandReloadmodule(Module *parent)
      : Command(parent, "RELOADMODULE", 1),
        evprov(parent, "event/reloadmodule"), dummyser(parent) {
    reloadevprov = &evprov;
    dummyserializer = &dummyser;
...
class DataKeeper final {
  ...
  struct ProviderInfo final {
    ...
    ProviderInfo(ModeHandler *mode) : itemname(mode->name), mh(mode) {}

    ProviderInfo(ExtensionItem *ei) : itemname(ei->name), extitem(ei) {}

    ProviderInfo(ClientProtocol::Serializer *ser)
        : itemname(ser->name), serializer(ser) {}
  };

  struct InstanceData final {
    /** Position of the ModeHandler or ExtensionItem that the serialized data
     * belongs to
     */
    size_t index;
  ...
  struct ModesExts {
    /** Mode data for the object, one entry per mode set by the module being
     * reloaded
     */
    std::vector<InstanceData> modelist;

    /** Extensions for the object, one entry per extension set by the module
     * being reloaded
     */
  ...
  struct OwnedModesExts : public ModesExts {
    ...
    OwnedModesExts(const std::string &Owner) : owner(Owner) {}
  };

  // Data saved for each channel
  struct ChanData final : public OwnedModesExts {
    /** Type of data stored for each member who has any affected modes or
     * extensions set
     */
    ...
    ChanData(Channel *chan) : OwnedModesExts(chan->name) {}
  };

  // Data saved for each user
  struct UserData final : public OwnedModesExts {
    static constexpr size_t UNUSED_INDEX = SIZE_MAX;
    size_t serializerindex;

    UserData(User *user, size_t serializeridx)
        : OwnedModesExts(user->uuid), serializerindex(serializeridx) {}
  };
  ...
  /** Get the index of a ProviderInfo representing the serializer in the
   * handledserializers list. If the serializer is not already in the list it is
   * added.
  ...
  /** Link previously saved extension names to currently available
   * ExtensionItems
   */
  void LinkExtensions();
   ...
   * The extensions are set directly on the extensible, the modes are added into
   * the provided mode change list.
   * @param data Data to unserialize from
   * @param extensible Object to restore
   * @param modetype MODETYPE_USER if the object being restored is a User,
   * MODETYPE_CHANNEL otherwise (for Channels and Memberships).
   ...
   * @param user User whose serializer to restore. If not local then calling
   * this method is a no-op.
   * @return True if the serializer didn't need restoring or was restored
   * successfully. False if the serializer should have been restored but the
   * required serializer is unavailable and the user was quit.
   */
   ...
   */
  void
  RestoreMemberData(Channel *chan,
                    const std::vector<ChanData::MemberData> &memberdatalist,
                    Modes::ChangeList &modechange);

  /** Verify that a service which had its data saved is available and owned by
   * the module that owned it previously
...
void DataKeeper::DoSaveUsers() {
  ...
  for (const auto &[_, user] : ServerInstance->Users.GetUsers()) {
    ...
    // Add to list if the user has any modes or extensions set that we are
    // interested in, otherwise we don't have to do anything with this user when
    // restoring
...
void DataKeeper::DoSaveChans() {
  ...
  for (const auto &[_, chan] : ServerInstance->Channels.GetChans()) {
    ...
    // Serialize all extensions attached to and all modes set on all members of
    // the channel
    SaveMemberData(chan, currmemberdata);

    // Same logic as in DoSaveUsers() plus we consider the modes and extensions
    // of all members
  ...
  for (const auto &[_, memb] : chan->GetUsers()) {
    for (size_t j = 0; j < handledmodes[MODETYPE_CHANNEL].size(); j++) {
      ...
      if ((pm) && (memb->HasMode(pm)))
        currdata.modelist.emplace_back(
            j, memb->user->uuid); // Need to pass the user's uuid to the mode
                                  // parser to set the mode later
    }
...
void DataKeeper::RestoreMemberData(
    Channel *chan, const std::vector<ChanData::MemberData> &memberdatalist,
    Modes::ChangeList &modechange) {
...
void DataKeeper::DoRestoreUsers() {
  ...
  for (const auto &userdata : userdatalist) {
    ...
    // Attempt to restore serializer first, if it fails it's a fatal error and
    // RestoreSerializer() quits them
    if (!RestoreSerializer(userdata.serializerindex, user))
      continue;

    RestoreObj(userdata, user, MODETYPE_USER, modechange);
    ServerInstance->Modes.Process(ServerInstance->FakeClient, nullptr, user,
...
void DataKeeper::DoRestoreChans() {
  ...
  for (const auto &chandata : chandatalist) {
    ...
    ServerInstance->Modes.Process(ServerInstance->FakeClient, chan, nullptr,
                                  modechange, ModeParser::MODE_LOCALONLY);
    modechange.clear();
...
class ReloadAction final : public InspIRCd::ActionBase {
  Module *const mod;
...
public:
  ReloadAction(Module *m, const std::string &uid,
      ...
      : mod(m), uuid(uid), passedname(passedmodname) {}
  ...
  void Call() override {
    ...
    if (result) {
      ...
      ServerInstance->SNO.WriteGlobalSno('a', "The {} module was reloaded.",
                                         passedname);
    } else {
      datakeeper.Fail();
      ServerInstance->SNO.WriteGlobalSno('a', "Failed to reload the {} module.",
                                         passedname);
    }
    ...
    if (user) {
      if (result)
        user->WriteNumeric(
            RPL_LOADEDMODULE, passedname,
            INSP_FORMAT("The {} module was reloaded.", passedname));
      else
        user->WriteNumeric(
            ERR_CANTUNLOADMODULE, passedname,
            INSP_FORMAT("Failed to reload the {} module.", passedname));
    }
  }
};
...
CmdResult CommandReloadmodule::Handle(User *user, const Params &parameters) {
  ...
  if (m == creator) {
    user->WriteNumeric(
        ERR_CANTUNLOADMODULE, parameters[0],
  ...
  if ((m) && (ServerInstance->Modules.CanUnload(m))) {
    ServerInstance->AtomicActions.AddAction(
        new ReloadAction(m, user->uuid, parameters[0]));
...
class CoreModReloadmodule final : public Module {
private:
